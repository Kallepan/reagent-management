...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, MatFormFieldModule, MatDatepickerModule, MatAutocompleteModule, MatNativeDateModule, MatInputModule, MatButtonModule, RouterModule],
...
export class LotsCreateComponent implements OnInit {
  ...
  private _filterType(types: BakType[], filterValue: string): BakType[] {
    return types.filter(
      (option) => option.name.toLowerCase().includes(filterValue) || option.producer.toLowerCase().includes(filterValue) || option.article_number?.toLowerCase().includes(filterValue),
    );
  }

  displayFn(type: BakType): string {
    if (!type) return '';
    return type && type.name ? `${type.name} - ${type.producer} - ${type.article_number}` : '';
  }
}
